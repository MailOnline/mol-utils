...
describe('dom', function () {
  ...
  describe('getPosition / isElementInViewport', function () {
    var testDiv, el, big1;
    ...
    beforeEach(function (){
      ...
      big1 = createEl('div');
      big1.style.height = '10000px'; // a big element allows the scrolling on the window
      document.body.appendChild(testDiv);
      testDiv.appendChild(el);
      testDiv.appendChild(big1);
      scrollToTop();
    });
    afterEach(function (){
      dom.remove(testDiv);
    ...
    it('must return the position of the passed element', function () {
      el.style.position = 'absolute';
      assert.deepEqual(dom.getPosition(el), {left: 10, top: 10, bottom: 110, right: 110});
      el.style.position = 'relative';
      assert.deepEqual(dom.getPosition(el), {left: 18, top: 18, bottom: 118, right: 118});
    });
    it('must check if the element is in the viewport', function () {
      assert.isTrue(dom.isElementInViewport(el, 0));
      window.scrollTo(0, 10);
      assert.isFalse(dom.isElementInViewport(el, 0));
      assert.isTrue(dom.isElementInViewport(el, 11));
    });
  });
