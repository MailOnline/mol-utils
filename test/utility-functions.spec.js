...
describe('utilities.snakeCase', function () {
  it('should convert to utilities.snakeCase', function () {
    assert.equal(utilities.snakeCase('ABC'), 'a_b_c');
    assert.equal(utilities.snakeCase('alanBobCharles'), 'alan_bob_charles');
  });


  it('should allow separator to be overridden', function () {
    assert.equal(utilities.snakeCase('ABC', '&'), 'a&b&c');
    assert.equal(utilities.snakeCase('facebookButton', '-'), 'facebook-button');
    assert.equal(utilities.snakeCase('alanBobCharles', '&'), 'alan&bob&charles');
  });
});
...
describe('utilities.throttle', function(){
  var clock;
  beforeEach(function (){
  ...
  it('must throttles a function', function(){
    var spy = sinon.spy();
    var inc = utilities.throttle(spy, 50);
    inc();
    setTimeout(inc, 20);
    setTimeout(inc, 25);
    clock.tick(70);
  ...
  it('must still preserve the the last function call if no more calls were done', function() {
    var spy = sinon.spy();
    var inc = utilities.throttle(spy, 50);
    inc(1); // must call this one
    inc(2); // must supress this one
    inc(3); // must call this one after 50 miliseconds
    ...
    assert.equal(spy.callCount, 2);
    clock.tick(70);
    assert.equal(spy.callCount, 2); // only to confirm
  });
});
...
describe('debounce', function(){
  ...
  it('must return a function', function(){
    assert.isFunction(utilities.debounce(utilities.noop, 10));
  });

  it('must delay the function call until the specified timeout has passed withing the fn been called', function(){
    var debounceFn = utilities.debounce(callback, 50);
    debounceFn();
    this.clock.tick(40);
    debounceFn();
  ...
  it('must pass the arguments', function(){
    var debounceFn = utilities.debounce(callback, 50);
    debounceFn(1, 2, 3 , 4);
    this.clock.tick(50);
    sinon.assert.calledWith(callback, 1, 2 , 3 , 4);
  });

  it('must be passible to call the function immediately after the first call happens', function(){
    var debounceFn = utilities.debounce(callback, 50, true);
    debounceFn();
    sinon.assert.calledOnce(callback);
    this.clock.tick(40);
    debounceFn();
...
describe('rotateRight', function () {
  it('must return rotated array', function () {
    assert.deepEqual(utilities.rotateRight([1, 2, 3], 1), [2, 3, 1]);
    assert.deepEqual(utilities.rotateRight([1, 2, 3], 0), [1, 2, 3]);
    assert.deepEqual(utilities.rotateRight([1, 2, 3], 2), [3, 1, 2]);
    assert.deepEqual(utilities.rotateRight([1, 2, 3], 3), [1, 2, 3]);
    assert.deepEqual(utilities.rotateRight([1, 2, 3], 5), [3, 1, 2]);
    assert.deepEqual(utilities.rotateRight([1, 2, 3], 61), [2, 3, 1]);
  });
});
...
describe('once', function () {
  it('must execute the passed handler once and only once no matter how many events we register to it.', function () {
    ...
    utilities.once(player, ['play', 'playing', 'pause'], spy);
    player.trigger('play');
...
describe('utilities.isMobile', function(){
  ...
  it('must return false it the userAgent is not from mobile', function(){
    utilities._UA = 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.0; Trident/4.0; WOW64; ' +
